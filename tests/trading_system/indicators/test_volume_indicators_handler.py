...
def simulate_handler(handler: TradingSystemHandler) -> None:
    tp.cast(TradingInterfaceMock, handler.ti).refresh()
    while handler.ti.is_alive():
        tp.cast(TradingInterfaceMock, handler.ti).update()
        handler.update()
        # False update
...
def test_accumulation_handler(candles: tp.List[Candle], result: tp.List[float]) -> None:
...
def test_on_balance_volume_handler(candles: tp.List[Candle], result: tp.List[float]) -> None:
    ti = TradingInterfaceMock(candles)
...
def test_relative_strength_index_handler(candles: tp.List[Candle], result: tp.List[float]) -> None:
    ti = TradingInterfaceMock(candles)
