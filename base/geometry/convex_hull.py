...
def build_convex_hull(points: np.ndarray, is_sorted: bool = False) -> \
    ...
    for i, point in enumerate(points[1:]):
        ...
        if i == len(points) - 2 or point_sign > 0:
            while len(upper_bound) > 1 and np.cross(
                    upper_bound[-1] - upper_bound[-2],
                    point - upper_bound[-2]) >= 0:
                upper_bound.pop()
            upper_bound.append(point)
        if i == len(points) - 2 or point_sign < 0:
            while len(lower_bound) > 1 and np.cross(
...
def get_upper_bound(points: np.ndarray, is_sorted: bool = False) -> np.ndarray:
    return build_convex_hull(points, is_sorted)[1]


def get_lower_bound(points: np.ndarray, is_sorted: bool = False) -> np.ndarray:
    return build_convex_hull(points, is_sorted)[0]


def put_line(points: np.ndarray) -> tp.Tuple[float, float]:
    model = LinearRegression().fit(points.T[0].reshape(len(points), 1),
                                   points.T[1])
